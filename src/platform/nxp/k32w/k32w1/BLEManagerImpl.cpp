...
//OSA_TASK_HANDLE_DEFINE(mHost_TaskId);
//OSA_TASK_DEFINE(Host_Task, HOST_TASK_PRIORITY, 1, HOST_TASK_STACK_SIZE, (uint8_t)FALSE);
...
#define CHIP_BLE_KW_EVNT_TIMEOUT 2000
...
OSA_EVENT_HANDLE_DEFINE(event_msg);
...
CHIP_ERROR BLEManagerImpl::_Init()
{
    ...
    osa_status_t osaStatus = KOSA_StatusSuccess;
    ...
    /* Initialization of message wait events - used for receiving BLE Stack events */
    osaStatus = OSA_EventCreate(event_msg, TRUE);
    VerifyOrExit(KOSA_StatusSuccess == osaStatus, err = CHIP_ERROR_INCORRECT_STATE);
    ...
    connectionTimeout =  xTimerCreate("bleTimeoutTmr", pdMS_TO_TICKS(CHIP_BLE_KW_CONN_TIMEOUT), 
                                      false, (void *) 0, blekw_connection_timeout_cb);
    ...
    /* BLE Host Stack Init */
    VerifyOrExit(Ble_HostInitialize(blekw_generic_cb, Hcit_SendPacket) == gBleSuccess_c, err = CHIP_ERROR_INCORRECT_STATE);
    ...
    VerifyOrExit(OSA_EventWait(event_msg, CHIP_BLE_KW_EVNT_INIT_COMPLETE, TRUE, CHIP_BLE_KW_EVNT_TIMEOUT, &flags) == KOSA_StatusSuccess,
    ...
    VerifyOrExit(GattServer_RegisterHandlesForWriteNotifications(1, attChipRxHandle) == gBleSuccess_c,err = CHIP_ERROR_INCORRECT_STATE);
    ...
    sbleAdvTimeoutTimer = xTimerCreate("BleAdvTimer", pdMS_TO_TICKS(100), false, (void *) this, BleAdvTimeoutHandler);
...
BLEManagerImpl::ble_err_t BLEManagerImpl::blekw_send_event(int8_t connection_handle, uint16_t handle, uint8_t * data, uint32_t len)
{
    ...
    /************* Send the indication *************/
    if (OSA_EventClear(event_msg, CHIP_BLE_KW_EVNT_INDICATION_CONFIRMED | CHIP_BLE_KW_EVNT_INDICATION_FAILED) != KOSA_StatusSuccess)
    {
    ...
    if (OSA_EventWait(event_msg, CHIP_BLE_KW_EVNT_INDICATION_CONFIRMED | CHIP_BLE_KW_EVNT_INDICATION_FAILED, FALSE,
                      CHIP_BLE_KW_EVNT_TIMEOUT, &event_mask) != KOSA_StatusSuccess)
    {
...
#else
    if (KOSA_StatusSuccess!= OSA_TaskCreate((osa_task_handle_t)mHost_TaskId, OSA_TASK(Host_Task), NULL)
    {
        return CHIP_ERROR_NO_MEMORY;
    }
#endif
                                                            ...
                                                            gapAdvertisingData_t * adv, gapScanResponseData_t * scnrsp)
{
    ...
    if (OSA_EventWait(event_msg, CHIP_BLE_KW_EVNT_RND_ADDR_SET, FALSE, CHIP_BLE_KW_EVNT_TIMEOUT, &event_mask) != KOSA_StatusSuccess)
    {
        return BLE_E_ADV_PARAMS_FAILED;
    }
...
CHIP_ERROR BLEManagerImpl::ConfigureAdvertisingData(void)
{
    ...
    adv_params.minInterval = advInterval;
    adv_params.maxInterval = advInterval;
    ...
    adv_params.channelMap   = (gapAdvertisingChannelMapFlags_t) (gGapAdvertisingChannelMapDefault_c);
...
void BLEManagerImpl::HandleWriteEvent(intptr_t arg)
{
    sInstance.HandleWriteEvent((blekw_msg_t *)arg);
    MSG_Free((blekw_msg_t *)arg);
}

void BLEManagerImpl::HandleDisconnectEvent(intptr_t arg)
{
...
void BLEManagerImpl::HandleAdvIntervalChange(intptr_t arg)
{
    // stop advertiser, change interval and restart it;
    sInstance.StopAdvertising();
    // restart only if requested, arg must be true
    if (arg)
    {
...
void BLEManagerImpl::bleAppTask(void * p_arg)
{
    while (1)
    {
        //todo. Call xEventGroupWaitBits function from k32w1 sdk
        //xEventGroupWaitBits(bleAppTaskLoopEvent, LOOP_EV_BLE, true, false, portMAX_DELAY);

        PlatformMgr().LockChipStack();

        //todo. Call MSG_Pending from k32w1 sdk
        if (1 /*MSG_Pending(&blekw_msg_list)*/)
        {
            /* There is message from the BLE tasks to solve */
            blekw_msg_t * msg = NULL;//(blekw_msg_t *) MSG_DeQueue(&blekw_msg_list);
...
void BLEManagerImpl::HandleConnectEvent(uint8_t connId)
{
    ChipLogProgress(DeviceLayer, "BLE is connected with device: %d.\n", connId);

    device_id = connId;
    blekw_start_connection_timeout();
    sInstance.AddConnection(connId);
    //mFlags.Set(Flags::kRestartAdvertising);
    //PlatformMgr().ScheduleWork(DriveBLEState, 0);
}
...
void BLEManagerImpl::HandleConnectionCloseEvent(uint8_t connId)
{
    ChipLogProgress(DeviceLayer, "BLE is disconnected with device: %d.\n", connId);

    if (sInstance.RemoveConnection(connId))
    {
        ChipDeviceEvent event;
        event.Type                           = DeviceEventType::kCHIPoBLEConnectionError;
...
void BLEManagerImpl::blekw_gap_connection_cb(deviceId_t deviceId, gapConnectionEvent_t * pConnectionEvent)
{
    ...
    if (pConnectionEvent->eventType == gConnEvtConnected_c)
    {
        ...
        sInstance.HandleConnectEvent(deviceId);
    ...
    else if (pConnectionEvent->eventType == gConnEvtDisconnected_c)
    {
        ...
        sInstance.HandleConnectionCloseEvent(deviceId);
...
{
    PlatformMgr().ScheduleWork(HandleDisconnectEvent, 0);
}
...
void BLEManagerImpl::blekw_gatt_server_cb(deviceId_t deviceId, gattServerEvent_t * pServerEvent)
{
    ...
    case gEvtMtuChanged_c:
        //Disabled for now, nobody uses the MTU
        //(void) Gatt_GetMtu(deviceId, &tempMtu);
        
        blekw_start_connection_timeout();
        ChipLogProgress(DeviceLayer, "BLE MTU size has been changed");
                                                     ...
                                                     uint16_t length)
{
    ...
    memcpy(att_wr_data->data, data, length);

    //todo. Call MSG_Queue function from k32w1 sdk
    /* Put message in the queue */
    if (1) //gListOk_c != MSG_Queue(&blekw_msg_list, msg))
    {
        assert(0);
    }
                                                     ...
                                                     uint16_t length)
{
    ...
    PlatformMgr().ScheduleWork(HandleWriteEvent, (intptr_t)msg);
    return CHIP_NO_ERROR;
}
...
CHIP_ERROR BLEManagerImpl::blekw_msg_add_att_read(blekw_msg_type_t type, uint8_t device_id, uint16_t handle)
{
    ...
    //todo. Call MSG_Alloc function from k32w1 sdk
    /* Allocate a buffer with enough space to store the packet */
    msg = NULL; //(blekw_msg_t *) MSG_Alloc(sizeof(blekw_msg_t) + sizeof(blekw_att_read_data_t));
...
CHIP_ERROR BLEManagerImpl::blekw_msg_add_u8(blekw_msg_type_t type, uint8_t data)
{
    ...
    //todo. Call MSG_Alloc function from k32w1 sdk
    /* Allocate a buffer with enough space to store the packet */
    msg = NULL;//(blekw_msg_t *) MSG_Alloc(sizeof(blekw_msg_t));
...
void BLEManagerImpl::BleAdvTimeoutHandler(TimerHandle_t xTimer)
{
    if (sInstance.mFlags.Has(Flags::kFastAdvertisingEnabled))
    {
        ...
        // Execute stop/start advertisment on CHIP task, here we are on the FreeRtos timer task
        PlatformMgr().ScheduleWork(HandleAdvIntervalChange, 1);

        // Slow advertise for 15 Minutes
        sInstance.StartBleAdvTimeoutTimer(CHIP_DEVICE_CONFIG_BLE_ADVERTISING_TIMEOUT); 
    }
    else if (sInstance._IsAdvertisingEnabled())
    {
        ...
        // Execute stop advertisment on CHIP task, here we are on the FreeRtos timer task
        PlatformMgr().ScheduleWork(HandleAdvIntervalChange, 0);
