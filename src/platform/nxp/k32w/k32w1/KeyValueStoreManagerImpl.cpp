...
#include <platform/nxp/k32w/k32w1/NXPConfig.h>
#include <set>
#include <string>
...
uint16_t GetStringKeyId(const char * key, uint16_t * freeId)
{
    ...
    {
        nvmInternalId = chip::DeviceLayer::Internal::NXPConfigKey(nvmIdKvsKey, keyId);
        err =
                                          ...
                                          size_t offset_bytes)
{
    ...
    if (keyId < kMaxNumberOfKeys)
    {
        ...
        nvmInternalId = chip::DeviceLayer::Internal::NXPConfigKey(nvmIdKvsValue, keyId);
