...
RTBdxOtaSender::RTBdxOtaSender()
{
    memset(mFilePath, 0, kFilepathMaxLength);
}

void RTBdxOtaSender::SetFilepath(const char * path)
{
    if (path != nullptr)
    {
...
CHIP_ERROR RTBdxOtaSender::InitializeTransfer(chip::FabricIndex fabricIndex, chip::NodeId nodeId)
{
    if (mInitialized)
    {
...
void RTBdxOtaSender::HandleTransferSessionOutput(TransferSession::OutputEvent & event)
{
    ...
    case TransferSession::OutputEventType::kQueryReceived: {
        ...
        blockData.IsEof  = (blockData.Length < blockSize) ||
            (mNumBytesSent + static_cast<uint64_t>(blockData.Length) == ota_fw_placeholder_bin_len || (mNumBytesSent + bytesToRead == ota_fw_placeholder_bin_len));
