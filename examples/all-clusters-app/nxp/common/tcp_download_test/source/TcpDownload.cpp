...
void _tcpDownload(void *pvParameters)
{
    ...
    ChipLogProgress(DeviceLayer, "[TCP_DOWNLOAD] Connecting to Wi-Fi network: SSID = '" WIFI_SSID "' and PASSWORD = '" WIFI_PASSWORD "'");
    ...
    // TODO should wait for connection establishment or failure (but the socket below will start accepting once WiFi is connected)
#endif
...
void EnableTcpDownloadComponent(void)
{
    ...
    if (xTaskCreate(&_tcpDownload, TCP_DOWNLOAD_TASK_NAME, TCP_DOWNLOAD_TASK_SIZE, NULL, TCP_DOWNLOAD_TASK_PRIO, &taskHandle) != pdPASS)
    {
