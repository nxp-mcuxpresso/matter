...
The Project CHIP K32W061 Light Switch Example demonstrates how to remotely control a bound
...
SDK, and supports remote access and control of a bound light bulb over a low-power,
...
-   [procedures for binding operation] (#binding)
...
light switch device, built using the Project CHIP codebase and the NXP K32W061
...
**Button SW3** can be used to change the state of the bound light bulb. This
...
user@ubuntu:~/Desktop/git/connectedhomeip/examples/light-switch-app/nxp/k32w/k32w0$ gn gen out/debug --args="k32w0_sdk_root=\"${NXP_K32W0_SDK_ROOT}\" chip_with_OM15082=1 chip_with_ot_cli=0 is_debug=false chip_crypto=\"platform\" chip_with_se05x=0 chip_pw_tokenizer_logging=false chip_detail_logging=false"
...
The resulting output file can be found in out/debug/chip-k32w0x-light-switch-example.
...
documentation with _out/debug/chip-k32w0x-light-switch-example.bin_ if DK6Programmer
is used or with _out/debug/chip-k32w0x-light-switch-example_ if MCUXpresso is used.
...
_out/debug/chip-k32w0x-light-switch-example-database.bin_ after a successful build.
...
```
python3 ../../../../../examples/platform/nxp/k32w/k32w0/scripts/detokenizer.py serial -i /dev/ttyACM0 -d out/debug/chip-k32w0x-light-switch-example-database.bin -o device.txt
```
...
```
DK6Programmer.exe -V2 -s <COM_PORT> -P 1000000 -Y -p FLASH@0x4000="chip-k32w0x-light-switch-example.bin"
```
...
## procedures for binding operation

1. commission light into fabric, the assigned light node id is 1
```
sudo ./chip-tool pairing ble-thread 1 hex:XXXXXXXX 20202021 3840
```
2. commission light switch into fabric, the assigned light switch node id is 2
```
sudo ./chip-tool pairing ble-thread 2 hex:XXXXXXXX 20202021 3840
```
3. configure the access control list on light node
```
sudo ./chip-tool accesscontrol write acl '[{"fabricIndex": 1, "privilege": 5, "authMode": 2, "subjects": [112233], "targets": null}, {"fabricIndex": 1, "privilege": 3, "authMode": 2, "subjects": [2], "targets": [{"cluster": 6, "endpoint": 1, "deviceType": null}, {"cluster": 8, "endpoint": 1, "deviceType": null}]}]' 1 0
```
4. configure the binding cluster on light switch node
```
sudo ./chip-tool binding write binding '[{"fabricIndex": 1, "node": 1, "endpoint": 1, "cluster": 6}, {"fabricIndex": 1, "node": 1, "endpoint": 1, "cluster": 8}]' 2 1
```
5. wait a few seconds for binding establishment
6. press the SW3 button on light switch board, and will see the D3 LED on and off on light board