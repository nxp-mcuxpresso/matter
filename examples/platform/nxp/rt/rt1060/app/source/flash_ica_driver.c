...
#if (!DISABLE_IMAGE_VERIFICATION && ENABLE_AWS_OTA)
#include "sln_auth.h"
#include "sln_flash_files.h"
#endif /* (!DISABLE_IMAGE_VERIFICATION && ENABLE_AWS_OTA) */
...
static uint8_t __attribute__((section(".bss.$SRAM_OC_CACHEABLE")))s_appImgBuffer[FLASH_SECTOR_SIZE];
...
static fica_t __attribute__((section(".bss.$SRAM_OC_NON_CACHEABLE")))s_fica = {0};
...
#if (!DISABLE_IMAGE_VERIFICATION && ENABLE_AWS_OTA)
int32_t FICA_Save_Certificate(int32_t imgType, uint8_t *certPem)
{
    int32_t status = SLN_FLASH_NO_ERROR;
    ...
    if (SLN_FLASH_NO_ERROR == status)
    {
        ...
        {
            status = sln_flash_fs_ops_save(CRED_SIGNING_CERT, certPem,
                                         safe_strlen((char *)certPem, MAX_CERT_LEN) + 1);
        }
        else if (FICA_IMG_TYPE_APP_A == imgType)
        {
            status = sln_flash_fs_ops_save(APP_A_SIGNING_CERT, certPem,
                                         safe_strlen((char *)certPem, MAX_CERT_LEN) + 1);
        }
        else if (FICA_IMG_TYPE_APP_B == imgType)
        {
            status = sln_flash_fs_ops_save(APP_B_SIGNING_CERT, certPem,
                                         safe_strlen((char *)certPem, MAX_CERT_LEN) + 1);
        }
        else
        {
...
int32_t FICA_Verify_Signature(int32_t imgType)
{
    ...
    if (SLN_FLASH_NO_ERROR == status)
    {
        ...
        status = SLN_Read_Flash_At_Address((msgFlashOffset + imageLen) & 0x0FFFFFFF, (uint8_t *)certHeader, sizeof(certHeader));
...
int32_t FICA_GetCurBank(int32_t *pImgBank)
{
    ...
    if (SLN_FLASH_NO_ERROR == status)
    {
        ...
        *pImgBank = s_fica.header.currType;
    }
...
int32_t FICA_bank_program_ext_init(fica_img_bank_t newImgBank)
{
    ...
    s_newAppImgStartAddr = (newImgBank == FICA_IMG_BANK_A) ? FICA_IMG_APP_A_ADDR : FICA_IMG_APP_B_ADDR;
    ...
    if (newImgBank == FICA_IMG_BANK_A)
    {
        if (curImgBank == FICA_IMG_BANK_A)
        {
            ...
            s_newAppImgStartAddr = FICA_IMG_APP_B_ADDR;
        }
    }
#else
    ...
    s_newAppImgMaxSize = FICA_IMG_APP_A_SIZE;
    s_newAppCurrLen    = 0;
...
{
    int32_t curBank  = FICA_IMG_TYPE_NONE;
    int32_t status   = SLN_FLASH_NO_ERROR;
...
int32_t FICA_GetBankStartingAddr(fica_img_bank_t bank, uint32_t *startBankAddr)
{
    ...
    *startBankAddr = 0;
    ...
    {
        *startBankAddr = FICA_IMG_APP_A_ADDR;
    }
    else if (bank == FICA_IMG_BANK_B)
    {
        *startBankAddr = FICA_IMG_APP_B_ADDR;
    }
