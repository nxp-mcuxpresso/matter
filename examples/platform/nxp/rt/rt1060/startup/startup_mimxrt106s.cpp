...
static void map_flexram(void)
{
    ...
    uint8_t dtcBankCount = dtcSize / 0x8000U;
    uint8_t itcBankCount = itcSize / 0x8000U;
    uint8_t ocBankCount = ocSize / 0x8000U - 16;
...
void ResetISR(void) {
    ...
    volatile unsigned int vCompilerStackTop = (unsigned int)&_vStackTop;
     volatile unsigned int vNewStackTop = vCompilerStackTop - vOldStackUsed;
    __DSB();
    __ISB();

    // Copy contents of old stack before setting new SP
     volatile  unsigned char *oldStackData = (unsigned char *)(vOldStackTop - 1);
     volatile  unsigned char *newStackData = (unsigned char *)(vCompilerStackTop - 1);
...
//     TODO - copy the .c, breakpoint and debug this
//     Copy the data sections from flash to SRAM.
